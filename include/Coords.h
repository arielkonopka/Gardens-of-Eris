...
public:
    ...
    double manhattan(const Coords &other) const;
    ...
    static Coords min(const Coords oth1, const Coords oth2);
    static Coords max(const Coords oth1, const Coords oth2);

    Coords validate(const Coords oth) const;
    Coords cutOff(const Coords oth) const;
...
namespace std {
template<>
struct hash<myUtility::Coords>
{
    size_t operator()(const myUtility::Coords &coords) const { return hash<int>()(coords.getX()) ^ (hash<int>()(coords.getY()) << 1); }
};
} // namespace std
#endif
