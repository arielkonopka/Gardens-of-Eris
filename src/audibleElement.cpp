...
void audibleElement::playSound(std::string eventType, std::string event)
{
    ...
    else if (this->getCollector().get()!=nullptr)
    {
...
void audibleElement::ps(std::shared_ptr<bElem>who, std::string eventType, std::string event)
{
    ...
    soundManager::getInstance()->registerSound(who->getBoard()->getInstanceId(),c3d,vel,this->getInstanceid(),this->getType(),this->getSubtype(),"Move",event);
}
