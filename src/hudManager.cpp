...
hudManager::hudManager()
{
    ...
    this->upperCorner = coords(configManager::getInstance()->getConfig()->tileWidth * 3,
                               int(configManager::getInstance()->getConfig()->tileHeight * 3));
    size = size + (upperCorner * 2); // we need space from both ends
    this->HUDGamefield = al_create_bitmap(size.x, size.y);
    this->HUDGlobal = al_create_bitmap(videoManager::getInstance().getHudSize().x,
                                       videoManager::getInstance().getHudSize().y);
