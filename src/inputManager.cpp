...
controlItem inputManager::translateEvent(ALLEGRO_EVENT* ev)
{
    ...
    if (this->pressed_keys[ALLEGRO_KEY_UP])
    {
        type=0;

        dir=UP;
    }
    if (this->pressed_keys[ALLEGRO_KEY_DOWN])
    {
        type=0;

        dir=DOWN;
    }
    if (this->pressed_keys[ALLEGRO_KEY_LEFT])
    {
        type=0;

        dir=LEFT;
    }
    if (this->pressed_keys[ALLEGRO_KEY_RIGHT])
    {
        dir=RIGHT;
        type=0;
    }
    if (this->pressed_keys[ALLEGRO_KEY_LSHIFT] && type>=0)
        type=1;
    if (this->pressed_keys[ALLEGRO_KEY_LCTRL]&& type>=0)
        type=2;
    if (this->pressed_keys[ALLEGRO_KEY_X])
        type=3;
    if (this->pressed_keys[ALLEGRO_KEY_LWIN] && type>=0)
        type=4;
